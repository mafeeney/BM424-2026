...
The current population of `r city_name` (as of `r census_year`) is `r sample(150000:170000, size=1)`. The demographic data for the city is shown below.
...
ggplot(data = birthdeath, aes(x=DemogYears, y=Num, group=Demog, colour=Demog)) + 
  ...
  scale_x_continuous(breaks = seq(min(DemogYears), max(DemogYears), by = 1)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
